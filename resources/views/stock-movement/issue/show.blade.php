...
<div class="card">
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th style="width:6%" class="text-end">Dự kiến</th>
                ...
                <tbody>
                    ...
                    @php
                    $tracking = (int)($detail->product?->tracking_type?->value ?? 1);
                    @endphp
                    <tr>
                        <td class="text-center text-body-secondary small">{{ $i + 1 }}</td>
                        <td>
                            <div class="fw-semibold small">{{ $detail->product?->code }} - {{ $detail->product?->name ?? '-' }}</div>
                        </td>
                        <td class="text-body-secondary small">{{ $detail->product?->specification ?? '-' }}</td>
                        <td class="text-body-secondary small">{{ $detail->uom?->name ?? '-' }}</td>
                        <td class="text-end small">{{ $fmt($detail->expected_qty) }}</td>
                        <td class="text-end small">
                            @if($detail->actual_qty !== null)
                            <span class="{{ $detail->actual_qty < $detail->expected_qty ? 'text-warning' : 'text-success' }}">
                        ...
                        <td class="small">{{ $detail->location ? $detail->location->code : '-' }}</td>
                        <td class="small">{{ $detail->receiver?->name ?? '-' }}</td>
                        <td class="small">{{ $detail->sn?->code ?? '-' }}</td>
                        <td class="small">
                            @if($detail->lot)
                            <span class="font-monospace">{{ $detail->lot->lot_number }}</span>
                            @elseif(in_array($tracking, [2,4]))
                            <span class="text-danger">Chưa có lot</span>
                            @else
                            <span class="text-body-secondary">-</span>
                            @endif
                        </td>
                        <td class="small">
                            @if($detail->serial)
                            <span class="font-monospace">{{ $detail->serial->serial_number }}</span>
                        ...
                        <td class="small">{{ $detail->sub_warehouse ?? '-' }}</td>
                        <td class="small">{{ $detail->note ?? '-' }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="14" class="text-center text-body-secondary py-5">
    ...
    <div class="card-footer d-flex justify-content-between align-items-center py-2">
        <small class="text-body-secondary">
            Tổng dòng: <strong>{{ $issue->details->count() }}</strong>
        </small>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Xác nhận đã xuất đủ hàng theo phiếu <strong>{{ $issue->code }}</strong>?</p>
            ...
            <div class="modal-footer">
                ...
                <form method="POST" action="{{ route('issues.complete', $issue) }}">
                    @csrf
