...
@section('title', (isset($stockOutRequest) ? 'Chỉnh sửa phiếu yêu cầu xuất' : 'Thêm phiếu yêu cầu xuất'))
...
@php
$isEdit = isset($stockOutRequest);
$action = $isEdit ? route('stock-out-requests.update', $stockOutRequest->id) : route('stock-out-requests.store');
@endphp

{{-- HEADER --}}
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-0 fw-semibold">{{ $isEdit ? 'Chỉnh sửa phiếu yêu cầu xuất' : 'Thêm phiếu yêu cầu xuất' }}</h4>
    </div>
    <a href="{{ $isEdit ? route('stock-out-requests.show', $stockOutRequest->id) : route('stock-requests.index') }}" class="btn btn-outline-secondary">
        Quay lại
    </a>
</div>

<form method="POST" action="{{ $action }}" id="stockOutRequestForm">
    @if ($errors->any())
                ...
                <table class="table table-hover align-middle mb-0" id="detailTable">
                    ...
                    <tbody id="detailBody">
                        ...
                        if (is_array($row)) {
                            ...
                            $actualQty = isset($row['actual_qty']) ? $row['actual_qty'] + 0 : '';
                            ...
                            $issueDate = $row->issue_date ? \Carbon\Carbon::parse($row->issue_date)->format('Y-m-d') : '';
                            $productCode = $row->product_code;
                            $productName = $row->product_name;
                            $quantity = $row->quantity + 0;
                            $uomName = $row->uom_name;
                            $actualQty = $row->actual_qty !== null ? $row->actual_qty + 0 : '';
                            $receiverId = $row->receiver_id;
                            $snCode = $row->sn_code;
                            $lotNumber = $row->lot_number;
                            $serialNumber = $row->serial_number;
    ...
    <div class="d-flex gap-2 justify-content-end mt-3">
        @if(!$isEdit)
        <button type="submit" id="stockOutRequestSubmitBtnNew" class="btn btn-outline-primary" name="action" value="save_and_new">
        ...
        @endif
        <button type="submit" id="stockOutRequestSubmitBtnSave" class="btn btn-primary" name="action" value="save">
            <span class="spinner-border spinner-border-sm me-1 d-none" role="status" aria-hidden="true"></span>
...
document.getElementById('stockOutRequestForm').addEventListener('submit', function(e) {
    if (submitting) {
        e.preventDefault();
        return;
    }
